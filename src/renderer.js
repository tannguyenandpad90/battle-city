...
export function draw(ctx, gameState) {
  ...
  for (const enemy of enemies) {
    if (enemy.alive) drawTank(ctx, enemy);
  }
...
function drawWater(ctx, x, y) {
  ...
  const offset = (waterFrame % 16) - 8;
  for (let i = 0; i < 4; i++) {
    const wx = x + (i * s / 4 + offset + s) % s;
    ctx.fillRect(wx, y + s / 4, s / 6, 2);
    ctx.fillRect(wx - 4, y + s * 3 / 4, s / 6, 2);
  }
}
 ...
 */
export function drawHUD(ctx, score, lives, level, enemiesLeft) {
  ctx.save();
  ...
  ctx.fillText('WASD/Arrows', x, CANVAS_HEIGHT - 80);
  ctx.fillText('SPACE: Fire', x, CANVAS_HEIGHT - 60);
  ctx.fillText('ENTER: Start', x, CANVAS_HEIGHT - 40);
